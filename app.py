...
from linebot.models import MessageEvent, TextMessage, TextSendMessagefrom
    ...
    if user_id not in user_states:
        ...
        if state == 'start':
            if text == '1':
                ...
                line_bot_api.reply_message(
                    event.reply_token,
                    TextSendMessage(text="你踏入藤蔓小路，四周變得越來越黑暗。隱約間，你聽到耳邊傳來低語聲，像是有人在訴說著不可思議的故事。地面上還出現了奇怪的腳印，你將會\n\n1：繼續前行，探索這條神秘的小路\n2：因感到不安，決定返回入口\n(請輸入1 or 2)")
                )
            elif text == '2':
                user_states[user_id] = 'bright_path'
                line_bot_api.reply_message(
                    event.reply_token,
                    TextSendMessage(text="光線充足的小路讓你感到安心，但漸漸地，你發現自己來到了一片空曠的草地。草地的盡頭有兩個出口，一個是陰暗的洞穴，另一個則是通向遠處明亮的出口，你選擇\n\n1：走向陰暗的洞穴，感到這裡可能藏著什麼秘密\n2：走向明亮的出口，期待光明的未來\n(請輸入1 or 2)")
                )
            ...
            if text == '1':
                ...
                line_bot_api.reply_message(
                    event.reply_token,
                    TextSendMessage(text="你進入了一個巨大的房間，房間中央擺滿了金光閃閃的寶藏。然而，當你踏進去時，地板開始發出刺耳的聲音，似乎隨時會崩塌，你選擇\n\n1：冒險跳向寶藏，試圖帶走這些財富\n2：立刻逃跑，保住自己的性命\n(請輸入1 or 2)")
                )
            elif text == '2':
